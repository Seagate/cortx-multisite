...
from s3replicationcommon.s3_range_read import S3AsyncGetRangeRead
...
async def main():
    ...
    object_range = config.object_range

    object_reader = S3AsyncGetRangeRead(session, request_id,
                                        bucket_name, object_name,
                                        object_range)

    reader_generator = object_reader.fetch()
    async for _ in reader_generator:
        pass

    logger.info("S3AsyncGetRangeRead test passed!")
    await session.close()
