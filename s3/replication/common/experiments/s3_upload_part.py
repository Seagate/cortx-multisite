...
async def data_generator(total_chunks):
    yield urandom(total_chunks)
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        for part_number in range(1, total_parts+1):
            data = data_generator(total_chunk)

            print('PUT on {} for object size {}.'.format(
                    config.endpoint + request_uri, total_chunk))

            query_params = urllib.parse.urlencode({'partNumber': part_number, 'uploadId': upload_id})
            body = ""
